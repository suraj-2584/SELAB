...
router.put('/availability/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  Room.findByIdAndUpdate(id, { $set: { isAvailable } })
    .then(data => res.status(200).json({ message: 'Room availability has been updated.', success: true }))
    .catch(err => res.json({ ...err, message: 'Failed to update Room availability.' }))
})
