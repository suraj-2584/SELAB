...
class RoomAction extends Component {
    ...
    async onAvailabilityChange(_id, isAvailable) {
        await axios.put(`/api/room/availability/${_id}`, { isAvailable: !isAvailable }).then(res => console.log(res)).catch(err => console.log(err));
        ...
        (!roomData.length || loading) ? (
            ...
            el =>
                <tr key={el._id} >
                    ...
                    <td>{el.isAvailable ?<button type="button" className="btn btn-danger" data-toggle="tooltip" data-placement="right" title="Click to set uncompleted"
                            onClick={() => this.onAvailabilityChange(el._id, el.isAvailable)}
                        >
                            Incomplete
                    </button>: <button type="button" className="btn btn-primary" data-toggle="tooltip" data-placement="right" title="Click to set completed"
                        onClick={() => this.onAvailabilityChange(el._id, el.isAvailable)}
                    >
                        Complete
                    </button>
